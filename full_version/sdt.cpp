...
void SDT::printSDTList(void)
{
	...
	for(auto list : sdt_info_list)
	{
		...
		std::printf("%3c %30s : %s   \n", '-', "service Provide Name", list.servicProvideeName.c_str());
		std::printf("%3c %30s : %s   \n", '-', "service Name", list.serviceName.c_str());

		i++;
	}
}

void SDT::saveSdtToJson(void)
{
	sdt_info_list.sort([](SDT_INFO_T& prog1, SDT_INFO_T& prog2) 
					-> bool { return prog1.program_number < prog2.program_number; });

	std::ofstream os("/Users/hskim/Sites/mpeg/sdt_json.txt");

	json_spirit::Array sdtArr;

	for(auto list : sdt_info_list)
	{
		json_spirit::Object sdtObj;

		sdtObj.push_back(json_spirit::Pair("version", list.version));
		sdtObj.push_back(json_spirit::Pair("section_number", list.section_number));
		sdtObj.push_back(json_spirit::Pair("service_id", list.program_number));
		sdtObj.push_back(json_spirit::Pair("EIT_schedule_flag", list.EIT_schedule_flag));
		sdtObj.push_back(json_spirit::Pair("EIT_present_following_flag", list.EIT_present_following_flag));
		sdtObj.push_back(json_spirit::Pair("running_status", list.running_status));
		sdtObj.push_back(json_spirit::Pair("free_CA_mode", list.free_CA_mode));
		sdtObj.push_back(json_spirit::Pair("servicProvideeName", list.servicProvideeName.c_str()));
		sdtObj.push_back(json_spirit::Pair("serviceName", list.serviceName.c_str()));

		sdtArr.push_back(sdtObj);
	}

	json_spirit::write_formatted(sdtArr, os);
