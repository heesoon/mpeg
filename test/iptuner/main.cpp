...
std::function<void(char*, size_t)> pCallBack = [&](char* _buffer, size_t num) {

	static unsigned long long cnt = 0;

	for(int i = 0; i < 7; i++)
	{
		char *buffer = &_buffer[188*i];
...
int main()
{
	...
	while(1)
	{
		...
		if( (option == 'b') || (option == 'B') )
		{
			...
			iptuner.registerCallBackFunc(pCallBack);
			iptuner.startTune(addr, port);
		}
		else if( (option == 'r') || (option == 'R') )
		{
			std::cout << "Restart Tune !!" << std::endl;
			std::string addr = "234.1.1.2";
			unsigned short port = 5000;
